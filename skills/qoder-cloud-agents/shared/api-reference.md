...
An org admin supplies an SA Key. Request scopes at token exchange, within the key's permissions. Use matching region/environment endpoints; the Global production pair is `https://openapi.qoder.sh` for exchange and `https://api.qoder.com` for business APIs. Never mix exchange and business endpoints from different regions.
...
For current authentication details, use [Qoder authentication](https://docs.qoder.com/cloud-agents/api/conventions/authentication).
