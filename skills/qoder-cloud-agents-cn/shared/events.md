...
Connect to `GET /sessions/{id}/events/stream` for real-time events. Since 2026-08-24, no `Last-Event-ID` means tail-only delivery, not historical replay. Establish SSE before triggering execution, or resume with an accepted/processed event ID; use List Events for history. For the current CN contract, see [CN Stream Events documentation](https://help.aliyun.com/zh/lingma/cloud-agents-cn/api-reference/stream-events). The tail-only behavior here is retained from the general workflow; importing CN domain references does not constitute live SSE verification.
